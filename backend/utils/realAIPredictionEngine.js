...
class RealAIPredictionEngine {
  ...
  // Update model based on actual results (machine learning)
  async updateModelWithResults(gameId, actualWinner, actualScore) {
    try {
      const prediction = this.gameHistory.find(p => p.gameId === gameId);
      if (!prediction) return;

      const wasCorrect = prediction.prediction.winnerId === actualWinner;
      const sport = prediction.sport.toLowerCase();

      // Update historical accuracy
      if (wasCorrect) {
        this.historicalAccuracy[sport] = Math.min(0.85, this.historicalAccuracy[sport] + 0.001);
      } else {
        this.historicalAccuracy[sport] = Math.max(0.45, this.historicalAccuracy[sport] - 0.002);
      }

      // Adjust weights based on performance
      if (!wasCorrect) {
        // Slightly adjust weights to improve future predictions
        const adjustment = 0.001;
        if (prediction.factors.recentForm.home > prediction.factors.recentForm.away && !wasCorrect) {
          this.weights.recentForm = Math.max(0.05, this.weights.recentForm - adjustment);
          this.weights.winPercentage = Math.min(0.35, this.weights.winPercentage + adjustment);
        }
      }

      console.log(`Model updated for game ${gameId}. Accuracy: ${sport} - ${(this.historicalAccuracy[sport] * 100).toFixed(1)}%`);
    } catch (error) {
